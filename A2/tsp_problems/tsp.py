...
#simulated annealing with three annealing schedules: linear decrease, 
#exponential decrease and division decrease
def simulated_annealing(cities):
    ...
    while T > 1:
	    ...
	    time += 1
  		...
  		time += 1
	'''
	T /= 1.01  #this is division decrease
	T *= 0.98  #this is exponential decrease
	'''
	T -= 1     #this is linear decrease (defult)
    return (best,current_cost)

def main():
   start_time = time.time()
   input()
   tour1,cost1,step1 = hill_climb(cities)
   tour2,cost2 = hill_climb_random_restart(cities)
   tour3,cost3 = simulated_annealing(cities)
